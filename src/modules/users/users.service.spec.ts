...
import { PaginationUsersDto, SelectUsersDto } from './dto';
...
describe('UsersService', () => {
  ...
  describe('selectAll', () => {
    ...
    it('should be return not found exception', async () => {
      const options = plainToClassFromExist(new SelectUsersDto(), { page: -1 });
  ...
  describe('updateOne', () => {
    it('should be return user entity', async () => {
      const received = await service.updateOne(expected, { password: 'some-secret' });
      expect(received).toBeInstanceOf(UserEntity);
